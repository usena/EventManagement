...
class Organizer_Win:
    def __init__(self, root):
        ...
        # Title Label
        lbl_title = Label(self.root, text="Add Organizer Details", font=("times new roman", 38, "bold"), bg="green", fg="white", bd=4, relief=RIDGE)
        lbl_title.place(x=0, y=0, width=1708, height=50)

        # User Details Frame
        labelframe_left = LabelFrame(self.root, bd=2, relief=RIDGE, text="Organizer Details", font=("times new roman", 12, "bold"), padx=2)
        labelframe_left.place(x=5, y=50, width=420, height=780)

        # User Reference
        lbl_organizer_ref = Label(labelframe_left, text="Organizer Ref", font=("arial", 12, "bold"), padx=2, pady=6)
        lbl_organizer_ref.grid(row=0, column=0, sticky=W)
        ...
        self.organizer_table = ttk.Treeview(details_table, columns=("ref", "name", "lname", "phone", "email"),
                                    xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        
        scroll_x.pack(side=BOTTOM, fill=X)
        scroll_y.pack(side=RIGHT, fill=Y)

        scroll_x.config(command=self.organizer_table.xview)
        scroll_y.config(command=self.organizer_table.yview)

        self.organizer_table.heading("ref", text="Organizer Ref")
        self.organizer_table.heading("name", text="Name")
        self.organizer_table.heading("lname", text="Last Name")
        self.organizer_table.heading("phone", text="Phone")
        self.organizer_table.heading("email", text="Email")

        self.organizer_table["show"] = "headings"
        self.organizer_table.column("ref", width=100)
        self.organizer_table.column("name", width=200)
        self.organizer_table.column("lname", width=200)
        self.organizer_table.column("phone", width=150)
        self.organizer_table.column("email", width=250)

        self.organizer_table.pack(fill=BOTH,expand=1)
        self.organizer_table.bind("<ButtonRelease-1>",self.get_cursor)
        self.fetch_data()
    ...
    def add_data(self):
        ...
        if  (not self.var_name.get().strip() or
            ...
            try:
                ...
                query = """
                        INSERT INTO Organizer (OrganizerName, OrganizerLName, OrganizerPhone, OrganizerEmail) 
                        VALUES (%s, %s, %s, %s)
                        """
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Success", "organizer has been added",parent=self.root)
    ...
    def fetch_data(self):
        ...
        if len(rows)!=0:
            self.organizer_table.delete(*self.organizer_table.get_children())
            for i in rows:
                self.organizer_table.insert("",END,values=i)
            conn.commit()
        conn.close()
    
    def get_cursor(self,event=""):
        cursor_row=self.organizer_table.focus()
        content=self.organizer_table.item(cursor_row)
        row=content["values"]
    ...
    def update(self):
        ...
        if (not self.var_name.get().strip() or
            ...
            try:
                ...
                query = """
                        UPDATE Organizer
                        SET OrganizerName=%s, OrganizerLName=%s, OrganizerPhone=%s, OrganizerEmail=%s
                        WHERE OrganizerID=%s
                        """
                values = (
                    self.var_name.get(),
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo("Update", "organizer details has been updated",parent=self.root)
            except mysql.connector.Error as err:
                # Parse and handle specific MySQL errors
                error_message = str(err)
    ...
    def mDelete(self):
        ...
        if mDelete>0:
            ...
            my_cursor=conn.cursor()
            query = """
                    DELETE FROM Organizer WHERE OrganizerID=%s
                    """
    ...
    def search(self):
        try:
            ...
            if search_field not in column_mapping:
                ...
                query = f"SELECT * FROM Organizer WHERE {column_mapping[search_field]} LIKE %s"
