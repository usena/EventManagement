    ...
    def add_data(self):
        if  (not self.var_name.get().strip() or
            ...
            try:
                ...
                elif "Enum value" in error_message:
                    messagebox.showerror("Database Error", "Venue Type must be either 'Indoor' or 'Outdoor'.", parent=self.root)

                # NOT NULL constraint violation
                elif "cannot be null" in error_message:
                    messagebox.showerror("Database Error", "All required fields must be filled.", parent=self.root)
